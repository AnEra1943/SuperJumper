...
/**
 * A progress bar is a horizontal indicator that allows a user to set a value.
 * The progress bar has a range (min, max) and a stepping between each value the
 * progress bar represents.
 * <p>
 * The preferred height of a progress bar is determined by the larger of the
 * knob and background. The preferred width of a progress bar is 140, a
 * relatively arbitrary size.
 */
public class ProgressBar extends Table {
     ...
     * specifies the distance between individual values. E.g. min could be 4,
     * max could be 10 and stepSize could be 0.2, giving you a total of 30
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        if (vertical) {
            ...
            if (knobBefore != null) {
                knobBefore.draw(batch, x + (int) ((width - knobBefore.getMinWidth()) * 0.5f), y, knobBefore.getMinWidth(), (int) (progressPos));
            }
        } else {
            ...
            if (knobBefore != null) {
                knobBefore.draw(batch, x, y + (int) ((height - knobBefore.getMinHeight()) * 0.5f), (int) (progressPos), knobBefore.getMinHeight());
            }
        }
     ...
     * clamped to the minumum and maximim values. {@link #clamp(float)} can be
     * overidden to allow values outside of the progress bars min/max range.
     *
