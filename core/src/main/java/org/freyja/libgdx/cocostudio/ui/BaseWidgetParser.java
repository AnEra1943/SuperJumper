...
public abstract class BaseWidgetParser {
    ...
    public Actor commonParse(CocoStudioUIEditor editor, ObjectData widget, Group parent, Actor actor) {
        ...
        if (actor instanceof Label) {
            Label tempActor = (Label) actor;
            tempActor.setFontScale(widget.getScale().getScaleX(), widget.getScale().getScaleY());
            //		tempActor.setSize(widget.getSize().getX() * widget.getScale().getScaleX(), widget.getSize().getY() * widget.getScale().getScaleY());
            tempActor.setOrigin(widget.getAnchorPoint().getScaleX() * actor.getWidth(), widget.getAnchorPoint().getScaleY() * actor.getHeight());
        } else {
        ...
        actor.setPosition(widget.getPosition().getX() - actor.getOriginX(), widget.getPosition().getY() - actor.getOriginY());
        if (widget.getRotationSkewX() != 0) {// CocoStudio 是顺时针方向旋转,转换下.
...
//		addActor(editor, actor, widget);
    ...
    public void addCallback(final Actor actor, final ObjectData widget) {
        if (widget.getCallBackType() == null || widget.getCallBackType().equals("")) {
            return;
        }
...
//	protected void addActor(CocoStudioUIEditor editor, Actor actor, ObjectData option) {
//
